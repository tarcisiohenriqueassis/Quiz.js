 ...
 function questoesSelecionadas() { 
  ...
  const perguntas = [
    ...
    const embaralhadas = perguntas.sort(() => Math.random() - 0.5); let acertos = 0; 
     
     // Iterando sobre as perguntas
     for (let i = 0; i < embaralhadas.length; i++) {
      
      console.log(`${i + 1} - ${embaralhadas[i].pergunta} `); 
      
      let respostaUsuario = entradaDados.question(" Resposta: "); 
      
       // Verificando se a resposta do usuário está correta 
       if (respostaUsuario.toLowerCase() === embaralhadas[i].resposta.toLowerCase()) {
         console.log(` Acertou!`); 
         acertos++; }
          else {
             console.log(" Errou!"); 
          ...
          console.log(`\n Total de acertos: ${acertos} de ${embaralhadas.length}`); 
        
          if (acertos <= 5) { 
            console.log(`Tente novamente! Você pode fazer melhor!`); 
        }
         else { console.log(`Parabéns! Você foi muito bem!!`); 
      } 
    }
    export {questoesSelecionadas};